...
class FgBrandController extends Controller
{
	/**
	...
	public function actionCreate()
	{
		...
		if (isset($_POST['FgBrand'])) {
			$model->attributes=$_POST['FgBrand'];
			if ($model->save()) {
	...
	public function actionIndex()
	{
		...
		$dataProvider=new CActiveDataProvider('FgBrand',array('criteria'=>$criteria));
	...
	{
		$model=new FgBrand('search');
		$model->unsetAttributes();  // clear any default values
		if (isset($_GET['FgBrand'])) {
			$model->attributes=$_GET['FgBrand'];
		}
	 ...
	 * @return FgBrand the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=FgBrand::model()->findByPk($id);
		$common = new CommonFunction();
		$common->record($model->id.'-'.$model->name);
		if ($model===null) {
	...
	public function selectItem(){
		...
		$fgAdvertiser = FgAdvertiser::model()->findAll();
		$selectItem['advertiser'] = CHtml::listData($fgAdvertiser,'id','name');
		return $selectItem;
	}
	/**
	 * Performs the AJAX validation.
	 * @param FgBrand $model the model to be validated
	 */
